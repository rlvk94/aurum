...
- **TermsVersion** / **terms_acceptance** — version-pinned Terms & Conditions documents (`src/server/terms/`) and an append-only log of each user's acceptance (verbatim text snapshot + hash + version + locale + timestamp). See the Terms & Conditions section below.
...
- Whenever you add, change, or remove a feature, validate whether the Terms & Conditions need to change (see Terms & Conditions below)

## Terms & Conditions

**Whenever you add, change, or remove a feature, validate whether the Terms & Conditions need to change as a result — before considering the work done.**

- T&C content lives in `src/server/terms/` as version-pinned documents (DA + EN).
- If the change affects anything the terms cover — what the app does with user data, what users are responsible for, billing, privacy, acceptable use, etc. — **append a new `TermsVersion`**. Never edit a shipped version's text in place: each user's accepted copy is snapshotted verbatim (in the append-only `terms_acceptance` table) and must stay accurate.
- Acceptances are recorded per `(user, version)`. To collect renewed consent after a terms change, ship the new version and re-gate users (reset onboarding, or add a dedicated re-consent gate in the protected layout — the schema already supports this).
...
- Edit a shipped Terms & Conditions version's text in place — append a new `TermsVersion` instead
